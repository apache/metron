...
public class HostFromPropertiesFileAdapterTest {
    ...
    private String sampleMessage = "{\"known_info\":{\"asset_value\":\"important\",\"type\":\"printer\",\"local\":\"YES\"}}";

    @Test
    public void testEnrich() throws Exception {

        Map<String, JSONObject> map_known_hosts = new HashMap<>();

        JSONArray jsonArray = (JSONArray) JSONValue.parse(sample_known_hosts);
        Iterator jsonArrayIterator = jsonArray.iterator();
        while(jsonArrayIterator.hasNext()) {
            JSONObject jsonObject = (JSONObject) jsonArrayIterator.next();
            String host = (String) jsonObject.remove("ip");
            map_known_hosts.put(host, jsonObject);
        }

        HostFromPropertiesFileAdapter hfa = new HostFromPropertiesFileAdapter(map_known_hosts);

        JSONObject hfaJson = hfa.enrich(new CacheKey("dummy", ip, null));
