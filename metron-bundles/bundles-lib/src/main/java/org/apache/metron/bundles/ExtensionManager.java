...
public class ExtensionManager {
   ...
   */
  public static void init(final List<Class> classes, final Bundle systemBundle,
  ...
  public ClassLoader createInstanceClassLoader(final String classType,
    ...
    // If the class is annotated with @RequiresInstanceClassLoading and the registered ClassLoader
    // is a URLClassLoader then make a new InstanceClassLoader that is a full copy of the BUNDLE
    // Class Loader, otherwise create an empty InstanceClassLoader that has the Bundle ClassLoader
    // as a parent
   ...
   * @return Set of Class
   * @throws NotInitializedException if the ExtensionManager has not been initialized
   */
  ...
  public ClassLoader removeInstanceClassLoaderIfExists(final String instanceIdentifier)
    ...
    checkInitialized();
    final ClassLoader classLoader = initContext.getInstanceClassloaderLookup()
        .remove(instanceIdentifier);
   ...
   */
  public boolean requiresInstanceClassLoading(final String classType)
      throws NotInitializedException {
    if (classType == null) {
   ...
   */
  public Bundle getBundle(final BundleCoordinates bundleCoordinates)
      throws NotInitializedException {
  ...
  /**
   * Returns the Extensions for a given Class.
  ...
  /**
   * Logs the current mappings for debugging.
   * @throws NotInitializedException if the ExtensionManager has not been initialized
   */
  public void logClassLoaderMapping() throws NotInitializedException {
    ...
    for (final Map.Entry<Class, Set<Class>> entry : initContext.getDefinitionMap().entrySet()) {
      ...
      for (final Class type : entry.getValue()) {
        final List<Bundle> bundles = initContext.getClassNameBundleLookup()
  ...
  /**
   * Verifies if the ExtensionManager has been initialized.
   * @throws NotInitializedException if it has not been
   */
