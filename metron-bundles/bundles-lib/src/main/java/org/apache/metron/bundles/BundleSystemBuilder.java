 ...
 * BundleProperties, if they are the only parameter must have archive extension and bundle extension
 * types properties present.
 *
 * <p>The {@link BundleSystemType} determines if a type other than the default should be
 * created.</p>
 */
public class BundleSystemBuilder {
  ...
  /**
   * Provide a SystemBundle.  If not provided with this override then the default SystemBundle will
   * be created.
   */
  ...
  private BundleSystem buildDefault() throws NotInitializedException {
    ...
    try {
      ...
      return new DefaultBundleSystem(fileSystemManager, extensionClasses, libFileObjects,
