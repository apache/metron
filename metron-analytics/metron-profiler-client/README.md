...
This project provides a client API for accessing the profiles generated by the [Profiler](../metron-profiler).  This includes both a Java API and Stellar API for accessing the profile data.  The primary use case is to extract profile data for use during model scoring.
...
Similarly, all six Client configuration parameters listed in the table below must match the Profiler configuration parameter settings from the time the profile was created. The period duration and other configuration parameters from the Profiler topology are stored in local filesystem at `$METRON_HOME/config/profiler.properties`. 

The Stellar Client API can be configured correspondingly by setting the following properties in Metron's global configuration, on local filesystem at `$METRON_HOME/config/zookeeper/global.json`, then uploaded to Zookeeper (at `/metron/topology/global`) by using `zk_load_configs.sh`: 
...
| profiler.client.row.key.builder       | The name of the RowKeyBuilder implementation class.                                                                                | Optional | org.apache.metron.profiler.hbase.SaltyRowKeyBuilder |
...
If in Profile, the groupBy criteria are: `[ “DAY_OF_WEEK()”, “URL_TO_PORT()” ]`.  Then in `PROFILE_GET`, an allowed `groups` value would be:  `[ “3”, “8080” ]`, which will select only records from Tuesdays with port number 8080.
