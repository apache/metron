...
describe('metron-alerts Search', function() {
  ...
  it('should delete first search items from search box having multiple search fields', () => {
    ...
    expect(page.getSearchText()).toEqualBcoz('enrichments:geo:ip_dst_addr:country:US AND source:type:bro', 'for search text US and bro');
    page.clickRemoveSearchChip();
    expect(page.getSearchText()).toEqualBcoz('source:type:bro', 'for search text bro after US is removed');
    page.clickRemoveSearchChip();
