...
public class KafkaControllerIntegrationTest {
    ...
    public void testSecurity() throws Exception {
        this.mockMvc.perform(post("/kafka/topic").with(csrf()).contentType(MediaType.parseMediaType("application/json;charset=UTF-8")).content(broTopic))
                .andExpect(status().isUnauthorized());

        this.mockMvc.perform(get("/kafka/topic/bro"))
                .andExpect(status().isUnauthorized());

        this.mockMvc.perform(get("/kafka/topic"))
                .andExpect(status().isUnauthorized());

        this.mockMvc.perform(get("/kafka/topic/bro/sample"))
                .andExpect(status().isUnauthorized());

        this.mockMvc.perform(delete("/kafka/topic/bro").with(csrf()))
                .andExpect(status().isUnauthorized());
    }
    ...
    public void test() throws Exception {
        ...
        this.mockMvc.perform(delete("/kafka/topic/bro").with(httpBasic(user,password)).with(csrf()))
                .andExpect(status().isNotFound());

        this.mockMvc.perform(post("/kafka/topic").with(httpBasic(user,password)).with(csrf()).contentType(MediaType.parseMediaType("application/json;charset=UTF-8")).content(broTopic))
        ...
        this.mockMvc.perform(get("/kafka/topic/bro").with(httpBasic(user,password)))
        ...
        this.mockMvc.perform(get("/kafka/topic/someTopic").with(httpBasic(user,password)))
                .andExpect(status().isNotFound());

        this.mockMvc.perform(get("/kafka/topic").with(httpBasic(user,password)))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.parseMediaType("application/json;charset=UTF-8")))
                .andExpect(jsonPath("$", Matchers.hasItem("bro")));


        this.mockMvc.perform(get("/kafka/topic/bro/sample").with(httpBasic(user,password)))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.parseMediaType("text/plain;charset=UTF-8")))
                .andExpect(jsonPath("$").isNotEmpty());

        this.mockMvc.perform(get("/kafka/topic/someTopic/sample").with(httpBasic(user,password)))
                .andExpect(status().isNotFound());
