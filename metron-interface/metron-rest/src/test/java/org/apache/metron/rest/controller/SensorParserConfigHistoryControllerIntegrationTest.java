...
public class SensorParserConfigHistoryControllerIntegrationTest {
    ...
    public void testSecurity() throws Exception {
        this.mockMvc.perform(get("/sensorParserInfo/broTest"))
                .andExpect(status().isUnauthorized());

        this.mockMvc.perform(get("/sensorParserInfo/getall"))
                .andExpect(status().isUnauthorized());

        this.mockMvc.perform(get("/sensorParserInfo/history/bro"))
                .andExpect(status().isUnauthorized());
    }
    ...
    public void test() throws Exception {
        ...
        SensorParserConfig bro2 = fromJson(broJson2);

        this.mockMvc.perform(get("/sensorParserConfigHistory/broTest").with(httpBasic(user1,password)))
                .andExpect(status().isNotFound());

        this.mockMvc.perform(post("/sensorParserConfig").with(httpBasic(user1,password)).with(csrf()).contentType(MediaType.parseMediaType("application/json;charset=UTF-8")).content(broJson1));
        Thread.sleep(1000);
        this.mockMvc.perform(post("/sensorParserConfig").with(httpBasic(user1,password)).with(csrf()).contentType(MediaType.parseMediaType("application/json;charset=UTF-8")).content(broJson2));
        Thread.sleep(1000);
        this.mockMvc.perform(post("/sensorParserConfig").with(httpBasic(user2,password)).with(csrf()).contentType(MediaType.parseMediaType("application/json;charset=UTF-8")).content(broJson3));
        ...
        String json = this.mockMvc.perform(get("/sensorParserConfigHistory/broTest").with(httpBasic(user1,password)))
        ...
        this.mockMvc.perform(get("/sensorParserConfigHistory").with(httpBasic(user1,password)))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.parseMediaType("application/json;charset=UTF-8")))
                .andExpect(jsonPath("$[?(@.config.sensorTopic == 'broTest')]").exists());

        this.mockMvc.perform(delete("/sensorParserConfig/broTest").with(httpBasic(user2,password)).with(csrf()));

        json = this.mockMvc.perform(get("/sensorParserConfigHistory/history/broTest").with(httpBasic(user1,password)))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.parseMediaType("application/json;charset=UTF-8")))
