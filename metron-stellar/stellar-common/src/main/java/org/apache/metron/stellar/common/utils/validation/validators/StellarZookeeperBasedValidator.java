...
public class StellarZookeeperBasedValidator extends BaseValidator {
  ...
  public Iterable<ValidationResult> validate() {
    // discover all the StellarZookeeperConfigurationProvider
    Set<StellarZookeeperConfigurationProvider> providerSet = new HashSet<>();

    for (Class<?> c : ClassIndex.getSubclasses(StellarZookeeperConfigurationProvider.class,
        Thread.currentThread().getContextClassLoader())) {
      boolean isAssignable = StellarZookeeperConfigurationProvider.class.isAssignableFrom(c);
      if (isAssignable) {
        try {
          StellarZookeeperConfigurationProvider reporter = StellarZookeeperConfigurationProvider.class
    ...
    providerSet.forEach((r) -> {
      try {
        List<StellarConfiguredStatementContainer> containers = r
            .provideContainers(client, (pathName, exception) -> {
