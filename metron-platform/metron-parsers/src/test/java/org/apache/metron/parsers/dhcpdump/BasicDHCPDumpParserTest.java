...
public class BasicDHCPDumpParserTest  extends TestCase {
     ...
     * The parser.
     */
    private static BasicDHCPDumpParser DHCPDumpParser = null;
    ...
    public void testSimpleMessage() throws ParseException, IOException {
        ...
        JSONObject keyValueJson = DHCPDumpParser.parse(rawMessage.getBytes()).get(0);

        Assert.assertEquals(keyValueJson.get("original_string").toString(), rawMessage);
        Assert.assertEquals(keyValueJson.get("timestamp").toString(), "1484584776581");

        Assert.assertEquals(keyValueJson.get("ip_src_addr").toString(), "100.100.100.177");
        Assert.assertEquals(keyValueJson.get("ip_dst_addr").toString(), "172.20.1.11");

        Assert.assertEquals(keyValueJson.get("time").toString(), "2017-01-16 17:39:36.581");
    }
