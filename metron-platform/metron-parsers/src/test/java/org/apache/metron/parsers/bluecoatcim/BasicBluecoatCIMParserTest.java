...
public class BasicBluecoatCIMParserTest {
	...
	public BasicBluecoatCIMParserTest() throws Exception {
		super();
	...
	public void testBluecoatCIMLine() {
		String testString = "<13>May 26 13:13:58 kdcproxy07p.kdc.capitalone.com bcproxysg|date=2016-05-26|time=1 13:58|duration=8|src_ip=10.83.41.26|user=-|cs_auth_group=-|x_exception_id=authentication_failed|filter_result=DENIED|category=\"Web Ads/Analytics\"|http_referrer=-|status=407|action=TCP_DENIED|http_method=CONNECT|http_content_type=-|cs_uri_scheme=tcp|dest=c1.adform.net|uri_port=443|uri_path=/|uri_query=-|uri_extension=-|http_user_agent=Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.87 Safari/537.36|dest_ip=38.65.20.26|bytes_in=302|bytes_out=215|x_virus_id=-|x_bluecoat_application_name=\"none\"|x_bluecoat_application_operation=\"none\"|target_ip=10.37.130.36|proxy_connect_type=Explicit|proxy_name=\"kdcproxy07p\"|proxy_ip=199.244.214.109|proxy_src_ip=-|dest_duration=-|x_rs_certificate_validate_status=-|x_rs_certificate_observed_errors=-|x_cs_ocsp_error=-|x_rs_ocsp_error=-|x_rs_connection_negotiated_ssl_version=-|x_rs_connection_negotiated_cipher=none|x_rs_connection_negotiated_cipher_size=-|x_rs_certificate_hostname=-|x_rs_certificate_hostname_category=-|x_cs_connection_negotiated_ssl_version=-|x_cs_connection_negotiated_cipher=none|x_cs_connection_negotiated_cipher_size=-|x_cs_certificate_subject=-|cs_realm=proxy_AD_realm|cs_auth_type=Certificate|x_auth_credential_type=-|%0D%0A ";
		...
		assertEquals(jo.get("dest"), "c1.adform.net");
		...
		assertEquals(jo.get("proxy_name"), "kdcproxy07p");
		assertEquals(jo.get("bytes_out"), "215");
		assertEquals(jo.get("dest_ip"), "38.65.20.26");
		...
		assertEquals(jo.get("http_user_agent"), "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/50.0.2661.87 Safari/537.36");
		assertEquals(jo.get("duration"), "8");
		assertEquals(jo.get("src_ip"), "10.83.41.26");
		assertEquals(jo.get("x_bluecoat_application_name"), "none");
		assertEquals(jo.get("timestamp").toString(), "1464268438000");
		assertEquals(jo.get("proxy_ip"), "199.244.214.109");
		assertEquals(jo.get("x_exception_id"), "authentication_failed");
		assertEquals(jo.get("x_bluecoat_application_operation"), "none");
		assertEquals(jo.get("target_ip"), "10.37.130.36");
		assertEquals(jo.get("uri_port"), "443");
