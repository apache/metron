...
public enum TransformationFunctions implements Function<List<Object>, Object> {
  ...
  TO_LONG(strings -> strings.get(0) == null?null: ConversionUtils.convert(strings.get(0), Long.class)),
  ...
  // math functions
  ADD(new MathFunctions.AddFunction()),
  SUB(new MathFunctions.SubtractFunction()),
  MULT(new MathFunctions.MultiplyFunction()),
  DIV(new MathFunctions.DivideFunction()),

  // control flow functions
  IF_THEN_ELSE(new ControlFlowFunctions.IfThenElseFunction()),
  EQUAL(new ControlFlowFunctions.EqualsFunction());
