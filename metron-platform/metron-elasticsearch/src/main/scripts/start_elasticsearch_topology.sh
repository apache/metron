...
if [ -d "$INDEXING_CONTRIB" ]; then
  export EXTRA_JARS="$INDEXING_CONTRIB/*"
  storm jar $METRON_HOME/lib/$TOPOLOGY_JAR org.apache.storm.flux.Flux --remote $METRON_HOME/flux/indexing/random_access/remote.yaml --filter $METRON_HOME/config/elasticsearch.properties --jars "$INDEXING_CONTRIB"
else
